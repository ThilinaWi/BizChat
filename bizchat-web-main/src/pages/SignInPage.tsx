...
const SignInPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await api<{ accessToken: string; refreshToken: string; user: { _id: string; firstName: string; lastName: string; email: string; role: string } }>('/auth/signin', {
        method: 'POST',
        body: { email, password },
      })
      ...
      if (message.toLowerCase().includes('fetch') || message.toLowerCase().includes('network') || message.toLowerCase().includes('connect')) {
      ...
      <section className="pt-32 pb-20 px-4">
        <div className="max-w-md mx-auto">
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            ...
            <Link to={`/signup?redirect=${encodeURIComponent(redirect)}`} className="text-[#FF6000] font-medium hover:underline">
              Sign up
            </Link>
          </p>
        </div>
